...
use crate::models::agents::agents_traits::{SpecialFunctions, FactSheet, ProjectScope};
...
impl AgentSolutionArchitect {
    ...
    async fn call_determined_external_urls(&mut self, factsheet: &mut FactSheet, msg_context: String) {
            ...
            match self.attributes.state {
                AgentState::Discovery => {
                    ...
                    if project_scope.is_external_url_required{
                        self.call_determined_external_urls(factsheet, factsheet.project_description.clone()).await;
                        self.attributes.state = AgentState::UnitTesting;
                    }
                }
...
mod tests {
    ...
    async fn tests_solution_architect() {
       let mut agent = AgentSolutionArchitect::new();
        ...
        agent.execute(& mut factsheet).await.expect("Unable to execute Solution Architect");
